import { MessageFlags, SlashCommandBuilder } from 'discord.js';
import { infoEmbed } from '../ui/embeds.js';

export const help = {
  data: new SlashCommandBuilder().setName('help').setDescription('Show quick-start help and command guide'),
  async execute(interaction: any) {
    const lines = [
      '**Input Requirements:** card 3-100 chars | max_price > 0 | grade up to 24 chars | priority NORMAL/HIGH/GRAIL | negative_keywords CSV (max 15)',
      '**Card Name Tip:** Casing does not matter. For best matches, include card number and grade when relevant.',
      '**Chases:** /chase-add · /chase-list · /chase-edit · /chase-remove',
      '**Alerts:** /alerts-settings (includes alert_currency) · /alerts-settings-reset · /alerts-recent',
      '**Plan:** /plan · /upgrade',
      '**Setup (Admin):** /setup-channel-set · /community-feed · /plan-set',
      '**Diagnostics:** /status · /chase-test'
    ];

    await interaction.reply({
      embeds: [
        infoEmbed(
          'Vaultr Help',
          `Start with /chase-add, then tune noise with /alerts-settings. Alerts are sent by DM when a listing matches.\n\n${lines.join('\n')}`
        )
      ],
      flags: MessageFlags.Ephemeral
    });
  }
};
