...
import {
  ...
  finalizeWeeklyDiscoveryAnalytics,
  rerankWeeklyDiscoveryReserve,
...
export type DiscoveryCandidate = {
  ...
  catalogFacts?: {
    source: LocalCardCatalogChoice['source'];
    sourceCardId: string;
    ...
    releaseYear?: number;
    imageUrl?: string;
    imageSourceKind?: LocalCardCatalogChoice['imageSourceKind'];
  };
  ...
  weeklyDiscovery?: WeeklyDiscoveryCandidateAnalysis;
};

type DiscoverySupplySource =
  | 'SOURCE_CATALOG'
  | 'USER_UNIVERSE'
  | 'GLOBAL_UNIVERSE'
  | 'MARKET_CACHE'
...
type WeeklyDiscoverySupplyStageCounts = {
  ...
  sourceBackedSuggestions: number;
  globalUniverseConsidered: number;
  userUniverseConsidered: number;
  deduplicatedCandidates: number;
  stableCanonicalIdCandidates: number;
  trustedImageCandidates: number;
  activeVaultEligibleCandidates: number;
  hardEligibleCandidates: number;
  marketResolvedEligibleCandidates: number;
  diversityEligibleCandidates: number;
  selectedCards: number;
};
type WeeklyDiscoverySupplyDiagnostics = {
  stages: WeeklyDiscoverySupplyStageCounts;
  projectedSelectedCount: number;
  projectedMarketResolvedCount: number;
  canonicalReserveCount: number;
  hardEligibleReserveCount: number;
  marketResolvedEligibleCount: number;
  ...
  countsByMarketStatus: Record<string, number>;
  rejectionCounts: Record<string, number>;
};
type WeeklyDiscoverySupplyReadiness = WeeklyDiscoverySupplyDiagnostics & {
  shouldTopOff: boolean;
...
type DiscoveryAssemblyStageDiagnostics = Pick<
  ...
  'rawGeneratedSuggestions' | 'sourceBackedSuggestions' | 'globalUniverseConsidered' | 'userUniverseConsidered' | 'deduplicatedCandidates'
>;
type WeeklyDiscoveryReservePruneReason =
  | 'NOT_DISPLAYABLE'
  | 'NOT_COLLECTOR_WORTHY'
  | 'ACTIVE_CHASE_ECHO'
  | 'VAULT_PARALLEL_PRINT'
  | 'UNTRUSTED_PRINTING_REFERENCE'
  | 'EXACT_REPEAT_COOLDOWN'
  | 'BELOW_CHASE_VALUE_FLOOR';
type WeeklyDiscoveryReservePruneDiagnostics = {
  inputCount: number;
  outputCount: number;
  rejectionCounts: Record<WeeklyDiscoveryReservePruneReason, number>;
  rejectionSamples: Record<WeeklyDiscoveryReservePruneReason, Array<{
...
type DiscoveryShelfValidationFailure = {
  code:
    ...
    | 'INSUFFICIENT_MARKET_RESOLVED';
  message: string;
  positions?: number[];
  details?: string[];
};
type DiscoveryShelfSelectionRejectionCode =
  | 'MISSING_CANONICAL_ID'
  | 'BAD_IMAGE'
  | 'BAD_DISPLAY_NAME'
  | 'DUPLICATE_CANONICAL_ID'
  | 'MISSING_RATIONALE'
  | 'REFERENCE_PRINTING_MISMATCH'
  | 'EXACT_REPEAT_COOLDOWN'
  | 'BELOW_CHASE_VALUE_FLOOR'
  | 'SUBJECT_SHELF_CAP'
  | 'FAMILY_SHELF_CAP'
...
type TopOffViabilityRejectionCode =
  | 'NOT_FINISHED'
  | 'NOT_DISPLAYABLE'
  | 'NOT_COLLECTOR_WORTHY'
  | 'ACTIVE_CHASE_ECHO'
...
type TopOffViabilityDiagnostics = {
  ...
  rejectionSamples: Record<TopOffViabilityRejectionCode, Array<{
    suggestionName: string;
    supplySource?: DiscoverySupplySource;
    lane: string;
    rejectionReason: TopOffViabilityRejectionCode;
    referenceSourceName?: string;
    hasReferenceImage: boolean;
    hasReferenceSourceCardId: boolean;
    canonicalCardId?: string;
...
type PersistValidatedWeeklyDiscoveryDropResult = {
  saved: boolean;
  itemCount: number;
  failures: DiscoveryShelfValidationFailure[];
  inspectedCount: number;
  qualifiedCount: number;
  marketResolvedCount: number;
  marketIncompleteCount: number;
...
type WeeklyMarketHydrationTargetDiagnostic = {
...
function logWeeklyDiscoveryReservePruneDiagnostics(
  userId: string,
  weeklyPeriod: string,
  stage: string,
  diagnostics: WeeklyDiscoveryReservePruneDiagnostics
): void {
  const activeRejectionCounts = Object.fromEntries(
    Object.entries(diagnostics.rejectionCounts).filter(([, count]) => count > 0)
  );
  const activeRejectionSamples = Object.fromEntries(
    Object.entries(diagnostics.rejectionSamples).filter(([, samples]) => samples.length > 0)
  );
  logWeeklyDiscoveryStage({
    ...
    counters: activeRejectionCounts
  });
  if (Object.keys(activeRejectionSamples).length > 0) {
    console.info(`[DiscoveryShelf] ${JSON.stringify({
      ...
      stage: `${stage}-samples`,
      status: 'SUCCESS',
      samples: activeRejectionSamples
    })}`);
  }
}
...
function firstBlockingMarketRecoveryMs(deadlineAtMs: number | undefined, fallbackMs = DISCOVERY_WEEKLY_MARKET_HYDRATION_STAGE_TIMEOUT_MS): number {
  const available = remainingBlockingMarketRecoveryMs(deadlineAtMs, fallbackMs);
  if (!deadlineAtMs) return available;
  return Math.max(0, Math.min(available, Math.floor(fallbackMs / 2)));
}
...
function summarizeWeeklyStageMetrics(stageMetrics: WeeklyDiscoveryStageMetric[]): string | undefined {
  if (stageMetrics.length === 0) return undefined;
  const slowestCompleted = [...stageMetrics]
    .filter((metric) => metric.status === 'SUCCESS')
    .sort((left, right) => right.elapsedMs - left.elapsedMs)[0];
  const lastMetric = stageMetrics[stageMetrics.length - 1];
  const parts: string[] = [];
  if (slowestCompleted) parts.push(`slowest completed stage ${slowestCompleted.stage} (${slowestCompleted.elapsedMs}ms)`);
  if (lastMetric && lastMetric !== slowestCompleted) parts.push(`last stage ${lastMetric.stage} (${lastMetric.status.toLowerCase()}, ${lastMetric.elapsedMs}ms)`);
  return parts.length > 0 ? parts.join('; ') : undefined;
}

function hasWeeklyOptionalStageBudget(deadlineAtMs: number | undefined, minimumMs = DISCOVERY_WEEKLY_OPTIONAL_STAGE_MIN_BUDGET_MS): boolean {
  if (!deadlineAtMs) return true;
  return remainingDeadlineMs(deadlineAtMs, minimumMs) >= minimumMs;
}
...
export function weeklyDiscoveryHasOptionalStageBudget(
  ...
  minimumMs = DISCOVERY_WEEKLY_OPTIONAL_STAGE_MIN_BUDGET_MS
): boolean {
  return hasWeeklyOptionalStageBudget(deadlineAtMs, minimumMs);
}
...
async function runDiscoveryAssemblySubstage<T>(
  ...
  return runWeeklyDiscoveryStage({
    ...
    stageMetrics: runtimeContext.stageMetrics
  }, async () => {
    const value = await work();
...
type WeeklyDiscoveryStageMetric = {
  stage: string;
...
const WEEKLY_DISCOVERY_FAMILY_CAP = Math.max(1, Number(process.env.WEEKLY_DISCOVERY_FAMILY_CAP ?? '3'));
const WEEKLY_DISCOVERY_FORMAT_CAP = Math.max(1, Number(process.env.WEEKLY_DISCOVERY_FORMAT_CAP ?? '4'));
const WEEKLY_DISCOVERY_LANE_CAP = Math.max(1, Number(process.env.WEEKLY_DISCOVERY_LANE_CAP ?? '5'));
...
export function sourceCatalogExpansionTargetCount(discoverySelectionCount: number, visibleCount: number): number {
  if (visibleCount >= DISCOVERY_SHELF_PAGE_SIZE) {
    return Math.min(
      discoverySelectionCount,
      Math.max(visibleCount + 36, Math.ceil(visibleCount * 2.8))
    );
  }
  return Math.min(
    discoverySelectionCount,
    Math.max(visibleCount + 4, visibleCount * 2)
  );
}

export function sourceCatalogSufficientSuggestionCount(discoverySelectionCount: number, visibleCount: number): number {
  if (visibleCount >= DISCOVERY_SHELF_PAGE_SIZE) {
    return Math.min(discoverySelectionCount, Math.max(visibleCount + 24, Math.ceil(visibleCount * 2.2)));
  }
  return Math.min(discoverySelectionCount, visibleCount + 2);
}
...
function extractRequestedCardNumber(value: string): string | undefined {
  const slash = /\b([A-Z]{0,4}\d{1,3}|\d{1,3})\s*\/\s*\d{1,3}\b/i.exec(value)?.[1];
  if (slash) return slash.replace(/\s+/g, '').toUpperCase();
  const promo = /\b((?:XY|SM|S|SV)-P\s*-?\s*\d{1,3}|\d{1,3}\s*\/\s*(?:XY|SM|S|SV)-P)\b/i.exec(value)?.[1];
  if (promo) return promo.replace(/\s+/g, '').replace(/-/g, '').toUpperCase();
  const coded = /\b(?:GG|TG|RC|XY|SM|SWSH|SVP|BW|DP|HGSS)\s?-?\d{1,4}\b/i.exec(value)?.[0];
  if (coded) return coded.replace(/\s|-/g, '').toUpperCase();
  const holo = /\bH\d{1,2}\b/i.exec(value)?.[0];
  if (holo) return holo.toUpperCase();
  return undefined;
}

function setHintForPrinting(value: string): string | undefined {
  const hints: Array<{ pattern: RegExp; value: string }> = [
    { pattern: /pokemon\s+151|\b151\b/i, value: '151' },
    { pattern: /paldean fates/i, value: 'Paldean Fates' },
    { pattern: /legendary treasures/i, value: 'Legendary Treasures' },
    { pattern: /xy black star/i, value: 'XY Black Star Promos' },
    { pattern: /sm black star/i, value: 'SM Black Star Promos' },
    { pattern: /bw black star/i, value: 'BW Black Star Promos' },
    ...
    { pattern: /gym heroes/i, value: 'Gym Heroes' },
    { pattern: /gym challenge/i, value: 'Gym Challenge' }
  ];
  return hints.find((hint) => hint.pattern.test(value))?.value;
}

function requestedLanguage(value: string): 'JAPANESE' | 'ENGLISH' | undefined {
  if (/\bjapanese\b|tcgdex japanese|sv-p|sm-p|\/p\b/i.test(value)) return 'JAPANESE';
  if (/\benglish\b|pokemon tcg\b|swsh black star|nintendo black star/i.test(value)) return 'ENGLISH';
  return undefined;
}
...
function resolvedReferenceCardNumber(candidate: DiscoveryCandidate): string | undefined {
  ...
  return lastSegment ? lastSegment.replace(/\s+/g, '').toUpperCase() : undefined;
}
...
export async function expandSourceBackedSuggestions(
  suggestions: DiscoverySuggestion[],
  activeChases: Chase[],
  tasteProfileChases: Chase[] = activeChases,
  targetCount = VISIBLE_DISCOVERY_COUNT,
  echoGuardChases: Chase[] = activeChases,
  options: { resolver?: typeof resolveSourceBackedDiscoveryCards } = {}
): Promise<DiscoverySuggestion[]> {
  const resolver = options.resolver ?? resolveSourceBackedDiscoveryCards;
  const expandedGroups = new Map<number, DiscoverySuggestion[]>();
  let nextIndex = 0;
  let timedOut = false;
  let timeoutHandle: NodeJS.Timeout | undefined;
  const targetPoolSize = Math.max(targetCount, Math.min(targetCount + 8, Math.ceil(targetCount * 1.25)));
  const sourceParentLimit = Math.min(suggestions.length, Math.max(targetCount + DISCOVERY_ENRICHMENT_CONCURRENCY, Math.ceil(targetCount * 1.5)));
  ...
  const hasEnoughDistinct = (): boolean => collectDistinct().length >= targetPoolSize;
  const worker = async () => {
    while (!timedOut && nextIndex < sourceParentLimit) {
      if (hasEnoughDistinct()) break;
      const index = nextIndex;
      const suggestion = suggestions[nextIndex];
      nextIndex += 1;
      const sourceBacked = await resolver(suggestion, activeChases, 12, tasteProfileChases);
...
function candidateFromCachedMarket(
  ...
  activeChases: Chase[] = [],
  refreshQueued = false,
  supplySource: DiscoverySupplySource = 'MARKET_CACHE'
): DiscoveryCandidate {
  if (!cacheEntry) return { suggestion, selectionIndex, sourceStatus: 'PENDING', supplySource };
  const listing = listingFromDiscoveryMarketCache(cacheEntry);
  if (listing && isActiveChaseEchoListing(listing, activeChases)) return { suggestion, selectionIndex, sourceStatus: 'PENDING', supplySource };
...
export function candidatesFromDiscoveryMarketCache(
  ...
  const marketCandidateRows = candidates.map((candidate, visibleIndex) => {
    ...
    const enrichedSuggestion: DiscoverySuggestion = {
      ...
      ...candidate.suggestion,
      canonicalReference: candidate.suggestion.canonicalReference ?? candidate.weeklyDiscovery?.canonicalReference ?? cacheBackedSuggestion.canonicalReference,
    ...
    if (refreshQueued) {
      ...
      if (context.queueRefreshes !== false) {
        refreshJobs.push({
          cacheKey,
  ...
  const acceptedRefreshKeys = context.queueRefreshes === false ? new Set<string>() : queueDiscoveryMarketRefreshes(refreshJobs);
  return marketCandidateRows.map((row) => {
    if (context.queueRefreshes === false) return row.candidate;
...
async function hydratePendingDiscoveryMarketCandidates(
  ...
  const selectedIndexes = new Set(selectedPendingCandidates.map(({ index }) => index));
  const diagnosticsByIndex = new Map<number, WeeklyMarketHydrationTargetDiagnostic>();
  for (const { candidate, index } of pendingCandidates) {
    ...
    if (options.diagnostics) {
      const diagnostic: WeeklyMarketHydrationTargetDiagnostic = {
        ...
        askingSampleCount: candidate.marketSampleSize,
        askingValue: candidate.typicalRawAskingTotal,
        soldSampleCount: candidate.soldSampleSize,
        soldValue: candidate.typicalRawSoldTotal,
        sourceStatusAfter: candidate.sourceStatus,
        afterStatus: candidateMarketStatus(candidate, context.targetCurrency),
        reason: selectedIndexes.has(index) ? 'hydration_pending' : 'not_selected_for_hydration'
      };
      options.diagnostics.push(diagnostic);
      diagnosticsByIndex.set(index, diagnostic);
    }
  }
  if (selectedPendingCandidates.length === 0) return cacheRefreshedCandidates;
  ...
  const currentCandidates = () => cacheRefreshedCandidates.map((candidate, index) => hydratedByIndex.get(index) ?? candidate);
  const maybeResolveEarly = () => {
    if (finished) return;
    if (!options.stopWhen?.(currentCandidates())) return;
    finished = true;
    resolveEarly?.();
  };

  const hydrationConcurrency = options.forceHydrateMissingMarket === true
  ...
  const workers = mapWithConcurrency(selectedPendingCandidates, hydrationConcurrency, async ({ candidate, index }) => {
    ...
    try {
      ...
      if (diagnostic) diagnostic.externalLookupAttempted = true;
      const hydrated = await enrichSuggestion(
        ...
        (askCandidate) => {
          ...
          upsertDiscoveryMarketCache({
            cacheKey,
            suggestionName: marketSuggestion.name,
      ...
      hydratedByIndex.set(index, {
        ...
        listing: hydrated.listing ?? candidate.listing,
        image: candidate.image?.sourceKind === 'CARD_REFERENCE' ? candidate.image : hydrated.image ?? candidate.image,
        sourceStatus: hydrated.sourceStatus === 'PENDING' ? undefined : hydrated.sourceStatus
      });
      const updatedCandidate = hydratedByIndex.get(index);
      if (diagnostic && updatedCandidate) {
        diagnostic.askingSampleCount = updatedCandidate.marketSampleSize;
        diagnostic.askingValue = updatedCandidate.typicalRawAskingTotal;
        diagnostic.soldSampleCount = updatedCandidate.soldSampleSize;
        diagnostic.soldValue = updatedCandidate.typicalRawSoldTotal;
        diagnostic.sourceStatusAfter = updatedCandidate.sourceStatus;
        diagnostic.afterStatus = candidateMarketStatus(updatedCandidate, context.targetCurrency);
        diagnostic.reason = marketHydrationNoReadyReason({
          attempted: true,
          beforeStatus: diagnostic.beforeStatus,
          afterStatus: diagnostic.afterStatus,
          soldSampleCount: diagnostic.soldSampleCount,
          askingSampleCount: diagnostic.askingSampleCount,
          sourceStatusAfter: diagnostic.sourceStatusAfter
        });
      }
      maybeResolveEarly();
    } catch {
      ...
      const diagnostic = diagnosticsByIndex.get(index);
      if (diagnostic) {
        diagnostic.externalLookupAttempted = true;
        diagnostic.reason = 'hydration_exception';
      }
    }
  });
  const deadline = new Promise<void>((resolve) => {
  ...
  const earlyCompletion = new Promise<void>((resolve) => {
    resolveEarly = resolve;
  });
  await Promise.race([workers.then(() => undefined), deadline, earlyCompletion]);
  finished = true;
  if (timeoutHandle) clearTimeout(timeoutHandle);

  for (const diagnostic of options.diagnostics ?? []) {
    if (diagnostic.reason !== 'hydration_pending') continue;
    diagnostic.reason = timedOut ? 'hydration_timed_out' : marketHydrationNoReadyReason({
      attempted: diagnostic.hydrationAttempted,
      beforeStatus: diagnostic.beforeStatus,
      afterStatus: diagnostic.afterStatus,
      soldSampleCount: diagnostic.soldSampleCount,
      askingSampleCount: diagnostic.askingSampleCount,
...
function discoveryMarketCacheDiagnosticStatus(entry: DiscoveryMarketCacheEntry | null): WeeklyMarketHydrationTargetDiagnostic['cacheStatusBefore'] {
  ...
  if (discoveryMarketCacheHasReliableEstimate(entry)) return 'READY';
  if (discoveryMarketCacheHasSignal(entry)) return 'THIN';
  return 'PENDING';
}

function marketHydrationNoReadyReason(input: {
  attempted: boolean;
  beforeStatus: DiscoveryScheduledMarketStatus;
  afterStatus: DiscoveryScheduledMarketStatus;
  soldSampleCount?: number;
  askingSampleCount?: number;
  sourceStatusAfter?: DiscoveryCandidate['sourceStatus'];
}): string {
  if (!input.attempted) return 'not_selected_for_hydration';
  if (input.afterStatus === 'READY') return 'became_ready';
  if (input.sourceStatusAfter === 'RATE_LIMITED') return 'provider_rate_limited';
  if (input.sourceStatusAfter === 'TIMEOUT') return 'provider_timeout';
  if (input.sourceStatusAfter === 'ERROR') return 'provider_error';
  if ((input.soldSampleCount ?? 0) > 0 || (input.askingSampleCount ?? 0) > 0) return 'insufficient_reliable_samples';
  if (input.beforeStatus === input.afterStatus) return 'no_new_market_evidence';
  return 'market_status_still_unready';
}
...
function hasPremiumCatalogTreatment(candidate: DiscoveryCandidate): boolean {
  const facts = candidate.catalogFacts;
  if (!facts) return false;
  return /\b(?:sar|sir|ar|ir|illustration|art rare|special illustration|secret|hyper|ultra|rainbow|gold star|shining|crystal|trainer gallery|galarian gallery|radiant collection|lv\.?\s*x|tag team|alt(?:ernate)? art|full art)\b/i.test([
    facts.rarity,
    facts.canonicalName,
    facts.setName,
...
function isOrdinaryModernCatalogPrinting(candidate: DiscoveryCandidate): boolean {
  const facts = candidate.catalogFacts;
  if (!facts) return false;
  if (facts.isPromo || hasPremiumCatalogTreatment(candidate)) return false;
  const setText = normalize([
    facts.series,
    facts.setId,
    facts.setName,
    facts.translatedSetName,
    facts.releaseYear?.toString()
  ].filter(Boolean).join(' '));
  const isVintageExEra = /\b(?:ex ruby|ex sandstorm|ex dragon|ex team|ex deoxys|ex emerald|ex unseen|ex delta|ex legend|ex holon|ex crystal|ex power|ex hidden)\b/.test(setText)
    || (!!facts.releaseYear && facts.releaseYear <= 2007);
  if (isVintageExEra) return false;
  const isModern = (!!facts.releaseYear && facts.releaseYear >= 2017)
    || hasModernSetContextText(setText)
    || /\b(?:sv|swsh|sm)\d*/.test(setText);
  if (!isModern) return false;
  const rarity = normalize(facts.rarity ?? '');
  const hasOrdinaryRarity = !rarity
    || /\b(?:common|uncommon|rare|holo rare|rare holo|double rare|rare ex|rare holo ex)\b/.test(rarity);
  if (!hasOrdinaryRarity) return false;
  return true;
}
...
function isCollectorWorthyWeeklyCandidate(candidate: DiscoveryCandidate): boolean {
  return !isOrdinaryModernCatalogPrinting(candidate)
    && !isLowValueModernFormatPromoCandidate(candidate)
...
export function backfillMarketReadyDiscoveryCandidates(
  ...
  let currentReadyShelfCount = 0;
  let currentDirectProfileReadyShelfCount = 0;
  let currentProfileMatchedReadyShelfCount = 0;
  const refreshBackfillCounts = (): void => {
    currentReadyShelfCount = readyShelfCount(merged);
    if (hasConcreteProfileSignals) {
      currentDirectProfileReadyShelfCount = directProfileReadyShelfCount(merged);
      currentProfileMatchedReadyShelfCount = profileMatchedReadyShelfCount(merged);
    }
  };
  refreshBackfillCounts();
  ...
  for (const entry of cacheEntries) {
    if ((!hasConcreteProfileSignals && currentReadyShelfCount >= targetCount) || (hasConcreteProfileSignals && currentDirectProfileReadyShelfCount >= targetCount)) break;
  ...
  for (const entry of cacheEntries) {
    if ((!hasConcreteProfileSignals && currentReadyShelfCount >= targetCount) || (hasConcreteProfileSignals && currentProfileMatchedReadyShelfCount >= targetCount)) break;
    if (isDiscoveryNameExcluded(entry.suggestionName, excludedNameKeys)) continue;
    const candidate = candidateFromCacheEntry(entry);
    if (!isDisplayableDiscoveryCandidate(candidate) || !isConcreteDiscoverySuggestion(candidate.suggestion)) continue;
    if (!isCollectorWorthyWeeklyCandidate(candidate)) continue;
  ...
  for (const entry of cacheEntries) {
    if (currentReadyShelfCount >= targetCount) break;
    if (isDiscoveryNameExcluded(entry.suggestionName, excludedNameKeys)) continue;
    const candidate = candidateFromCacheEntry(entry);
    if (!isCollectorWorthyWeeklyCandidate(candidate)) continue;
...
function isValuableEraTraitCacheCandidate(candidate: DiscoveryCandidate, profileTraits: Set<string>): boolean {
  if (!profileTraits.has('e-reader') && !profileTraits.has('wotc')) return false;
  const text = normalize([sourceCardText(candidate), candidate.suggestion.lane, candidate.suggestion.referenceSourceName, candidate.image?.sourceName].filter(Boolean).join(' '));
  const hasCandidateEra = /\be[- ]?reader\b|\bexpedition\b|\baquapolis\b|\bskyridge\b|\bbase set\b|\bteam rocket\b|\bgym heroes\b|\bgym challenge\b|\bneo\b|\bwizards\b/.test(text);
  if (!hasCandidateEra) return false;
  const marketTotal = discoveryMarketTotal(candidate);
  return /\bh\d{1,2}\b/.test(text) || (marketTotal !== undefined && marketTotal >= 100);
}

function traitAlignedReliableMarketCacheCandidates(
  context: {
    activeChases: Chase[];
    destination?: { country?: string; postalCode?: string };
    targetCurrency: SupportedCurrency;
  ...
  const positiveChases = positiveTasteSubjectChases(tasteProfileChases);
  if (positiveChases.length === 0) return [];
  const profileTraits = distinctProfileKeys(positiveChases, profileTraitKeys);
  const hasStrongTraitProfile = ['e-reader', 'wotc', 'japanese', 'promo', 'art', 'corocoro', 'publication']
    .some((trait) => profileTraits.has(trait));
  if (!hasStrongTraitProfile) return [];

  const excludedNameKeys = discoveryExclusionNameKeys(excludedNames);
  const selected: DiscoveryCandidate[] = [];
  const seenNames = new Set<string>();
  const seenVariantFamilies = new Set<string>();
  for (const entry of listReliableDiscoveryMarketCacheEntries({
    displayCurrency: context.targetCurrency,
    destinationCountry: context.destination?.country,
    limit: 1000
  })) {
    if (selected.length >= targetCount) break;
    const candidate = candidateFromReliableMarketCacheEntry(entry, context.targetCurrency, context.activeChases);
    if (isDiscoveryNameExcluded(candidate.suggestion.name, excludedNameKeys)) continue;
    if (isActiveChaseEchoSuggestion(candidate.suggestion, repeatGuardChases)) continue;
    if (!isDisplayableDiscoveryCandidate(candidate) || !isConcreteDiscoverySuggestion(candidate.suggestion)) continue;
    if (!hasReliableMarketEstimate(candidate) || !isMarketEstimateInRange(candidate, context.range)) continue;
    if (!isCollectorWorthyWeeklyCandidate(candidate)) continue;
    if (!hasCollectorShapedScheduledSignal(candidate)) continue;
    if (
      !hasCollectorProfileTraitMatch(candidate, positiveChases)
      && !isAdjacentThemeNoveltyCandidate(candidate, positiveChases)
      && !isValuableEraTraitCacheCandidate(candidate, profileTraits)
    ) continue;
    const nameKey = discoveryDisplayNameKey(candidate.suggestion.name);
    const variantKey = candidateVariantFamilyKey(candidate);
  ...
  return orderCandidatesForMarketConfidence(selected, tasteProfileChases);
}
...
function tasteOnlyCandidate(
  suggestion: DiscoverySuggestion,
  selectionIndex: number,
  supplySource: DiscoverySupplySource = 'SOURCE_CATALOG'
): DiscoveryCandidate {
  return {
    suggestion,
    selectionIndex,
...
const EVOLUTION_FAMILY_GROUPS: string[][] = [
...
const PARALLEL_PRINT_SET_FAMILIES: Array<{ key: string; tokens: string[] }> = [
  { key: 'pokemon-151', tokens: ['151', 'pokemon card 151', 'pokemon 151', 'sv2a'] },
  { key: 'crown-zenith-vstar-universe', tokens: ['crown zenith', 'vstar universe', 's12a'] },
  { key: 'paldean-fates-shiny-treasure-ex', tokens: ['paldean fates', 'shiny treasure ex', 'sv4a'] },
  { key: 'prismatic-evolutions-terastal-festival', tokens: ['prismatic evolutions', 'terastal festival', 'sv8a'] },
  { key: 'evolving-skies-eevee-heroes', tokens: ['evolving skies', 'eevee heroes', 's6a'] },
  { key: 'obsidian-flames-ruler-of-the-black-flame', tokens: ['obsidian flames', 'ruler of the black flame', 'sv3'] },
  { key: 'surging-sparks-super-electric-breaker', tokens: ['surging sparks', 'super electric breaker', 'sv8'] },
  { key: 'twilight-masquerade-crimson-haze', tokens: ['twilight masquerade', 'crimson haze', 'sv5a'] },
  { key: 'temporal-forces-wild-force-cyber-judge', tokens: ['temporal forces', 'wild force', 'cyber judge', 'sv5k', 'sv5m'] }
];
...
function sourceCatalogueSetLabelsForChases(chases: Chase[]): string[] {
  return uniqueValuesPreservingOrder(
    positiveTasteSubjectChases(chases)
      .map((chase) => setHintForPrinting([chase.cardName, chase.queryName, chase.targetNote].filter(Boolean).join(' ')))
      .filter((value): value is string => !!value)
  );
}
...
export function profileVariantSourceBackfillParents(chases: Chase[], targetCount = DISCOVERY_CANDIDATE_POOL_SIZE): DiscoverySuggestion[] {
  ...
  for (const setLabel of sourceCatalogueSetLabelsForChases(chases)) {
    parents.push({
      name: `${setLabel} source-backed Pokemon cards`,
      lane: 'Set Companion Trail',
      laneWhy: `trusted catalogue companions from ${setLabel}`,
      why: `branches from a set already present in this Vault into other collector-relevant cards from the same release`,
      nearby: [],
      evidenceSearchTerm: `${setLabel} Pokemon card`,
      evidenceAliases: [`${setLabel} Pokemon`, `${setLabel} cards`],
      requiredEvidenceTokens: ['set companion'],
      sourceTasteTokens: [`set:${setLabel}`, 'set companion'],
      curiosityScore: 8
    });
  }
...
function persistDiscoveryUniverseCandidate(candidate: DiscoveryCandidate): void {
  ...
  if (isGenericDiscoveryCardTitle(candidate.suggestion.name)) return;
  if (!candidate.suggestion.why?.trim() || !candidate.suggestion.laneWhy?.trim()) return;
  const canonicalId = candidate.suggestion.referenceSourceCardId?.trim() || candidate.image?.sourceCardId?.trim();
  if (!canonicalId) return;
  const canonicalImage = scheduledShelfImageFromCandidate(candidate);
  if (!canonicalImage || canonicalImage.sourceKind !== 'CARD_REFERENCE') return;
  upsertDiscoveryUniverseCard({
    canonicalName: candidate.suggestion.name,
    suggestion: candidate.suggestion,
    lane: candidate.suggestion.lane,
    sourceName: canonicalImage.sourceName ?? candidate.suggestion.referenceSourceName,
    imageUrl: canonicalImage.url ?? candidate.suggestion.referenceImageUrl,
...
function candidateFromDiscoveryUniverseCard(
  entry: DiscoveryUniverseCard,
  selectionIndex: number,
  supplySource: DiscoverySupplySource = 'GLOBAL_UNIVERSE'
): DiscoveryCandidate {
...
function topOffCandidateKey(candidate: DiscoveryCandidate): string {
  return candidate.suggestion.referenceSourceCardId?.trim()
    ?? candidate.image?.sourceCardId?.trim()
    ?? discoveryDisplayNameKey(candidate.suggestion.name);
}

function preCanonicalTopOffCandidateKey(candidate: DiscoveryCandidate): string {
  return candidate.suggestion.referenceSourceCardId?.trim()
    ?? candidate.image?.sourceCardId?.trim()
    ?? discoveryDisplayNameKey(candidate.suggestion.name);
}

function reserveCandidatePreferenceScore(candidate: DiscoveryCandidate, currency: SupportedCurrency): number {
  let score = 0;
  if (candidateHasCanonicalTrustedReference(candidate, currency)) score += 1000;
  else if (candidateHasTrustedPrintingReference(candidate, currency)) score += 700;
  else if (candidateHasTrustedCatalogueImage(candidate, currency)) score += 400;
  else if (candidateHasStableCanonicalId(candidate, currency)) score += 200;
  if (candidateMarketStatus(candidate, currency) === 'READY') score += 80;
  else if (candidateMarketStatus(candidate, currency) === 'THIN') score += 20;
  if (!isMarketplaceStyleDiscoveryName(candidate.suggestion.name) && !isGenericDiscoveryCardTitle(candidate.suggestion.name)) score += 30;
  if (candidate.weeklyDiscovery?.canonicalReference) score += 20;
  score += topOffCandidateSourcePriority(candidate);
  score += Math.min(12, candidate.soldSampleSize ?? candidate.marketSampleSize ?? 0);
  return score;
}

export function sanitizeWeeklyDiscoveryReserve(
  candidates: DiscoveryCandidate[],
  currency: SupportedCurrency
): DiscoveryCandidate[] {
  const bestByKey = new Map<string, { candidate: DiscoveryCandidate; index: number; score: number }>();
  for (const [index, candidate] of candidates.entries()) {
    if (!candidateHasStableCanonicalId(candidate, currency) && !candidateHasTrustedCatalogueImage(candidate, currency)) continue;
    const key = topOffCandidateKey(candidate);
    const score = reserveCandidatePreferenceScore(candidate, currency);
    const current = bestByKey.get(key);
    if (!current || score > current.score || (score === current.score && index < current.index)) {
      bestByKey.set(key, { candidate, index, score });
    }
  }
...
function emptyWeeklyDiscoveryReservePruneDiagnostics(inputCount: number): WeeklyDiscoveryReservePruneDiagnostics {
  return {
    inputCount,
    outputCount: 0,
    rejectionCounts: {
      NOT_DISPLAYABLE: 0,
      NOT_COLLECTOR_WORTHY: 0,
      ACTIVE_CHASE_ECHO: 0,
      VAULT_PARALLEL_PRINT: 0,
      UNTRUSTED_PRINTING_REFERENCE: 0,
...
function weeklyDiscoveryReservePruneSample(
  candidate: DiscoveryCandidate,
  currency: SupportedCurrency
): WeeklyDiscoveryReservePruneDiagnostics['rejectionSamples'][WeeklyDiscoveryReservePruneReason][number] {
  ...
  return {
    ...
    imageSourceKind: candidate.image?.sourceKind,
    hasReferenceImage: !!(candidate.suggestion.referenceImageUrl || candidate.image?.url),
    hasReferenceSourceCardId: !!(candidate.suggestion.referenceSourceCardId || candidate.image?.sourceCardId),
    canonicalCardId: candidate.suggestion.referenceSourceCardId?.trim() ?? candidate.image?.sourceCardId?.trim(),
    marketStatus: candidateMarketStatus(candidate, currency),
    evaluatedPrice: estimate?.amount,
    evaluatedCurrency: estimate?.currency
  };
}

function recordWeeklyDiscoveryReservePruneRejection(
  diagnostics: WeeklyDiscoveryReservePruneDiagnostics | undefined,
  candidate: DiscoveryCandidate,
  currency: SupportedCurrency,
  reason: WeeklyDiscoveryReservePruneReason
): void {
  if (!diagnostics) return;
  diagnostics.rejectionCounts[reason] += 1;
  if (diagnostics.rejectionSamples[reason].length < 3) {
    diagnostics.rejectionSamples[reason].push(weeklyDiscoveryReservePruneSample(candidate, currency));
  }
}

function weeklyDiscoveryReservePruneReason(
  candidate: DiscoveryCandidate,
  currency: SupportedCurrency,
  repeatHistory: Map<string, ExactRepeatHistoryEntry>,
  vaultEntries: ParallelPrintVaultEntry[],
  activeVaultChases: Chase[]
): WeeklyDiscoveryReservePruneReason | undefined {
  if (!isDisplayableDiscoveryCandidate(candidate)) return 'NOT_DISPLAYABLE';
  if (!isCollectorWorthyWeeklyCandidate(candidate)) return 'NOT_COLLECTOR_WORTHY';
  if (isActiveChaseEchoSuggestion(candidate.suggestion, activeVaultChases)) return 'ACTIVE_CHASE_ECHO';
  if (candidateParallelPrintRejection(candidate, vaultEntries)) return 'VAULT_PARALLEL_PRINT';
  if (!candidateHasTrustedPrintingReference(candidate, currency)) return 'UNTRUSTED_PRINTING_REFERENCE';
  const canonicalId = candidate.suggestion.referenceSourceCardId?.trim() ?? candidate.image?.sourceCardId?.trim();
  const repeat = canonicalId ? repeatHistory.get(canonicalId) : undefined;
  if (repeat && repeat.mostRecentIndex < WEEKLY_DISCOVERY_EXACT_REPEAT_COOLDOWN_SHELVES) return 'EXACT_REPEAT_COOLDOWN';
  const estimate = reliableWeeklyDiscoveryMarketEstimate(candidate, currency);
  if (estimate && estimate.amount < weeklyDiscoveryValueFloor(currency)) return 'BELOW_CHASE_VALUE_FLOOR';
  return undefined;
}

function sanitizePreCanonicalTopOffCandidates(
  candidates: DiscoveryCandidate[],
  currency: SupportedCurrency
): DiscoveryCandidate[] {
  const bestByKey = new Map<string, { candidate: DiscoveryCandidate; index: number; score: number }>();
  for (const [index, candidate] of candidates.entries()) {
    const key = preCanonicalTopOffCandidateKey(candidate);
    const score = reserveCandidatePreferenceScore(candidate, currency)
      + (candidate.supplySource === 'TOP_OFF_SOURCE_CATALOG' ? 300 : 0);
    const current = bestByKey.get(key);
    if (!current || score > current.score || (score === current.score && index < current.index)) {
      bestByKey.set(key, { candidate, index, score });
    }
...
function prunePublicationImpossibleReserve(
  candidates: DiscoveryCandidate[],
  currency: SupportedCurrency,
  recentDrops: ScheduledDiscoveryDrop[],
  activeVaultChases: Chase[],
  ...
  for (const candidate of candidates) {
    const reason = weeklyDiscoveryReservePruneReason(candidate, currency, repeatHistory, vaultEntries, activeVaultChases);
    if (reason) {
      recordWeeklyDiscoveryReservePruneRejection(diagnostics, candidate, currency, reason);
      continue;
    }
    kept.push(candidate);
  }
  if (diagnostics) diagnostics.outputCount = kept.length;
  return kept;
}

function weeklyPublicationSeedCandidates(discovery: { candidates: DiscoveryCandidate[]; weeklyReserveCandidates?: DiscoveryCandidate[] }): DiscoveryCandidate[] {
  return discovery.weeklyReserveCandidates && discovery.weeklyReserveCandidates.length > discovery.candidates.length
    ? discovery.weeklyReserveCandidates
    : discovery.candidates;
}

function topOffCandidateSourcePriority(candidate: DiscoveryCandidate): number {
  switch (candidate.supplySource) {
    ...
    case 'SOURCE_CATALOG':
      return 24;
    case 'TOP_OFF_GLOBAL_UNIVERSE':
    case 'GLOBAL_UNIVERSE':
      return 18;
    case 'TOP_OFF_MARKET_CACHE':
    case 'MARKET_CACHE':
      return 8;
    default:
      return 0;
...
function topOffParentSubjectKeys(suggestion: DiscoverySuggestion): string[] {
  return uniqueValuesPreservingOrder(
    universeContextTokensFromSuggestion(suggestion)
...
const GENERIC_TOP_OFF_PARENT_SUBJECT_KEYS = new Set([
  'collector',
  'e reader',
  'english',
  'exclusive',
...
function topOffParentSpecificSubjectKeys(suggestion: DiscoverySuggestion): string[] {
  return uniqueValuesPreservingOrder(
    [
      ...(suggestion.requiredEvidenceTokens ?? []),
      ...(suggestion.sourceTasteTokens ?? []),
      ...
      .flatMap((value) => profileSubjectTokens(value))
      .map((token) => normalizedSubjectIdentity(token))
      .filter((token): token is string => !!token && !GENERIC_TOP_OFF_PARENT_SUBJECT_KEYS.has(token))
      .slice(0, 4)
  );
}
...
function topOffParentWouldOverflowSubjectCap(
  ...
  const subjectKeys = topOffParentSubjectKeys(suggestion);
  return subjectKeys.length > 0
    && subjectKeys.every((key) => (readiness.selectedSubjectCounts[key] ?? 0) >= WEEKLY_DISCOVERY_SUBJECT_CAP);
}

function prioritizeTopOffSourceParents(
  parents: DiscoverySuggestion[],
  readiness: WeeklyDiscoverySupplyReadiness,
  profileChases: Chase[]
): DiscoverySuggestion[] {
  const preferredSubjectKeys = new Set(
    positiveTasteSubjectChases(profileChases)
      .flatMap((chase) => chaseSpecificSubjectTokens(chase))
      .map((token) => normalizedSubjectIdentity(token))
      .filter((token): token is string => !!token)
  );
  const japaneseTarget = hasJapaneseWeightedProfile(profileChases)
  ...
  const isJapaneseTopOffParent = (suggestion: DiscoverySuggestion): boolean => {
    ...
    return /\bjapanese\b|tcgdex|corocoro|vstar universe|terastal festival|eevee heroes|shiny treasure ex|sv\d+[a-z]?|s\d+[a-z]?/i.test(text);
  };
  const scoredParents = [...parents]
    .filter((suggestion) => !topOffParentWouldOverflowSubjectCap(suggestion, readiness))
    .sort((left, right) => {
    const score = (suggestion: DiscoverySuggestion): number => {
      let value = 0;
      const subjectKeys = topOffParentSubjectKeys(suggestion);
      if (subjectKeys.some((key) => preferredSubjectKeys.has(key))) value += 18;
      for (const key of subjectKeys) {
        const currentCount = readiness.selectedSubjectCounts[key] ?? 0;
        if (currentCount === 0) value += 16;
        else if (currentCount >= WEEKLY_DISCOVERY_SUBJECT_CAP) value -= 40;
        else value -= currentCount * 8;
      }
  ...
  const specificParents = scoredParents.filter((suggestion) =>
    topOffParentSpecificSubjectKeys(suggestion).some((key) => (readiness.selectedSubjectCounts[key] ?? 0) < WEEKLY_DISCOVERY_SUBJECT_CAP)
  );
  if (specificParents.length >= Math.min(24, Math.ceil(scoredParents.length * 0.4))) {
    const specificNames = new Set(specificParents.map((suggestion) => discoveryDisplayNameKey(suggestion.name)));
    return [
      ...specificParents,
      ...scoredParents.filter((suggestion) => !specificNames.has(discoveryDisplayNameKey(suggestion.name)))
    ];
  }
...
function prefilterBoundedTopOffBackfillCandidates(
  candidates: DiscoveryCandidate[],
  readiness: WeeklyDiscoverySupplyReadiness,
  currency: SupportedCurrency,
  recentDrops: ScheduledDiscoveryDrop[],
...
function topOffViabilitySample(
  candidate: DiscoveryCandidate,
  rejectionReason: TopOffViabilityRejectionCode
): TopOffViabilityDiagnostics['rejectionSamples'][TopOffViabilityRejectionCode][number] {
  const estimate = reliableWeeklyDiscoveryMarketEstimate(candidate, candidate.displayCurrency ?? 'CAD');
  return {
    suggestionName: candidate.suggestion.name,
    supplySource: candidate.supplySource,
    lane: candidate.suggestion.lane,
    rejectionReason,
    referenceSourceName: candidate.suggestion.referenceSourceName ?? candidate.image?.sourceName,
    hasReferenceImage: !!(candidate.image?.url || candidate.suggestion.referenceImageUrl),
    hasReferenceSourceCardId: !!(candidate.suggestion.referenceSourceCardId || candidate.image?.sourceCardId),
    canonicalCardId: candidate.suggestion.referenceSourceCardId?.trim() ?? candidate.image?.sourceCardId?.trim(),
    marketStatus: candidateMarketStatus(candidate, candidate.displayCurrency ?? 'CAD'),
    evaluatedPrice: estimate?.amount,
    evaluatedCurrency: estimate?.currency
  };
}

function recordTopOffViabilityRejection(
  diagnostics: TopOffViabilityDiagnostics | undefined,
  candidate: DiscoveryCandidate,
  code: TopOffViabilityRejectionCode
): void {
  if (!diagnostics) return;
  diagnostics.rejectionCounts[code] += 1;
  if (diagnostics.rejectionSamples[code].length < 3) {
    diagnostics.rejectionSamples[code].push(topOffViabilitySample(candidate, code));
  }
}

function boundedTopOffCandidateRejectionReason(
  candidate: DiscoveryCandidate,
  currency: SupportedCurrency,
  repeatHistory: Map<string, ExactRepeatHistoryEntry>,
  activeVaultChases: Chase[]
  ...
  if (!isDisplayableDiscoveryCandidate(candidate)) return 'NOT_DISPLAYABLE';
  if (!isFinishedShelfCandidate(candidate)) return 'NOT_FINISHED';
  if (!isCollectorWorthyWeeklyCandidate(candidate)) return 'NOT_COLLECTOR_WORTHY';
  if (isActiveChaseEchoSuggestion(candidate.suggestion, activeVaultChases)) return 'ACTIVE_CHASE_ECHO';
  if (!candidateHasTrustedPrintingReference(candidate, currency) && candidate.supplySource !== 'TOP_OFF_SOURCE_CATALOG') {
    return 'UNTRUSTED_REFERENCE';
  }
  if (candidateParallelPrintRejection(candidate, parallelPrintVaultEntries(activeVaultChases))) return 'VAULT_PARALLEL_PRINT';
  const canonicalId = candidate.suggestion.referenceSourceCardId?.trim() ?? candidate.image?.sourceCardId?.trim();
  if (!canonicalId && candidate.supplySource !== 'TOP_OFF_SOURCE_CATALOG') return 'MISSING_CANONICAL_ID';
  if (canonicalId) {
...
function topOffCandidateScore(
  ...
  if (hasConcreteProfileSubjectMatch(candidate, positiveTasteSubjectChases(profileChases))) score += 28;
  if (hasCollectorProfileTraitMatch(candidate, profileChases)) score += 16;
  if (isJapaneseDiscoveryCandidate(candidate) && hasJapaneseWeightedProfile(profileChases)) {
    const currentJapaneseCount = readiness.selectedCandidates.filter((entry) => isJapaneseDiscoveryCandidate(entry)).length;
    if (currentJapaneseCount < Math.max(2, Math.floor(DISCOVERY_WEEKLY_DROP_SIZE * 0.15))) score += 20;
  }
...
function selectDeficitAwareTopOffCandidates(
  ...
  for (const candidate of [...candidates]
    ...
    const rejectionReason = boundedTopOffCandidateRejectionReason(candidate, currency, repeatHistory, activeVaultChases);
    if (rejectionReason) {
...
function localCatalogChoiceLane(choice: LocalCardCatalogChoice, role: WeeklyDiscoveryRole): string {
  if (role === 'CORE_MATCH') return 'Collector Compass';
  if (localCatalogChoiceIsPromo(choice)) return 'Promo Trail';
  if (choice.language === 'ja') return 'Japanese Collector Trail';
  if (/\b(?:expedition|aquapolis|skyridge|e[- ]?reader)\b/i.test([choice.setName, choice.translatedSetName, choice.series].filter(Boolean).join(' '))) return 'E-Reader Era Trail';
  if (localCatalogChoiceHasPremiumTreatment(choice)) return 'Artwork Trail';
...
function localCatalogChoiceToDiscoveryCandidate(
  ...
  const hasPremiumTreatment = localCatalogChoiceHasPremiumTreatment(choice);
  const lane = localCatalogChoiceLane(choice, role);
  const anchorDetail = role === 'CORE_MATCH'
    ? 'the same Pokemon'
    : isPromo
      ? 'promo or special-release traits'
      : hasPremiumTreatment
        ? 'premium print treatment'
        : choice.language === 'ja'
          ? 'Japanese catalog traits'
  ...
  const baseCandidate: DiscoveryCandidate = {
    ...
    catalogFacts: {
      ...
      releaseEvent: choice.releaseEvent,
      releaseYear: choice.releaseYear,
      imageUrl: choice.imageUrl,
...
async function expandWeeklyDiscoveryCanonicalSupplyTopOff(input: {
  ...
  const excludedNames = uniqueValuesPreservingOrder([
    ...freshExcludedNames,
  ...
  const canRunSourceCatalogTopOff = hasWeeklyOptionalStageBudget(
    deadlineAtMs,
  ...
  const trustedUniverseBackfillCandidates = prefilterBoundedTopOffBackfillCandidates(
    sanitizePreCanonicalTopOffCandidates(uniqueTopOffCandidatesByIdentity([
      ...rawUserUniverseCandidates,
      ...rawGlobalUniverseCandidates
    ]), discovery.settings.alertCurrency),
    readiness,
    discovery.settings.alertCurrency,
    recentDrops,
  ...
  const boundedUniverseBackfill = orderCandidatesForCollectorPresentation(
    trustedUniverseBackfillCandidates,
    discovery.tasteProfileChases,
    Math.max(targetCount * 2, 24),
    discovery.negativeProfile,
  ...
  const marketBackedTopOffCandidates = candidatesFromDiscoveryMarketCache(rawTopOffCandidates, {
    ...marketContext,
    forceRefreshMissingSignal: readiness.marketResolvedShortfall > 0,
    forceRefreshThinSignal: readiness.marketResolvedShortfall > 0
  });
  const topOffDiagnostics = emptyTopOffViabilityDiagnostics();
  const prioritizedTopOff = selectDeficitAwareTopOffCandidates(
    marketBackedTopOffCandidates,
    readiness,
    discovery.tasteProfileChases,
    discovery.chases,
    discovery.settings.alertCurrency,
  ...
  logWeeklyDiscoveryStage({
    ...
    counters: {
      ...
      rejectedNotFinished: topOffDiagnostics.rejectionCounts.NOT_FINISHED,
      rejectedNotDisplayable: topOffDiagnostics.rejectionCounts.NOT_DISPLAYABLE,
      rejectedNotCollectorWorthy: topOffDiagnostics.rejectionCounts.NOT_COLLECTOR_WORTHY,
      rejectedActiveChaseEcho: topOffDiagnostics.rejectionCounts.ACTIVE_CHASE_ECHO,
      rejectedUntrustedReference: topOffDiagnostics.rejectionCounts.UNTRUSTED_REFERENCE,
  ...
  const topOffRejectionSamples = Object.entries(topOffDiagnostics.rejectionSamples)
    .filter(([, samples]) => samples.length > 0)
    .map(([reason, samples]) => ({
      reason,
      samples
    }));
  if (topOffRejectionSamples.length > 0) {
    console.info(`[DiscoveryShelf] ${JSON.stringify({
      event: 'WEEKLY_DISCOVERY_TOP_OFF_REJECTIONS',
      userId,
      weeklyPeriod,
      stage: 'topoff-dedup-selection',
      samples: topOffRejectionSamples
    })}`);
  }
  ...
  return {
    ...
    diagnostics: {
      rawGeneratedSuggestions: 0,
      sourceBackedSuggestions: rawSourceCandidates.length,
      globalUniverseConsidered: rawGlobalUniverseCandidates.length,
      userUniverseConsidered: rawUserUniverseCandidates.length,
      deduplicatedCandidates: rawTopOffCandidates.length
    }
  };
}
...
function topOffSuggestionIdentityKey(suggestion: DiscoverySuggestion): string {
  const sourceCardId = suggestion.referenceSourceCardId?.trim();
  if (sourceCardId) return `source:${sourceCardId}`;
  const sourceImageUrl = suggestion.referenceImageUrl?.trim();
  if (sourceImageUrl) return `image:${sourceImageUrl}`;
  return `name:${discoveryNameKey(suggestion.name)}`;
}

function uniqueTopOffSuggestionsByIdentity(suggestions: DiscoverySuggestion[]): DiscoverySuggestion[] {
  const seenKeys = new Set<string>();
  const uniqueSuggestions: DiscoverySuggestion[] = [];
  for (const suggestion of suggestions) {
    const key = topOffSuggestionIdentityKey(suggestion);
...
function uniqueTopOffCandidatesByIdentity(candidates: DiscoveryCandidate[]): DiscoveryCandidate[] {
  const seenKeys = new Set<string>();
  const uniqueCandidates: DiscoveryCandidate[] = [];
  for (const candidate of candidates) {
    const key = preCanonicalTopOffCandidateKey(candidate);
    if (!key || seenKeys.has(key)) continue;
...
function sourceCardText(candidate: DiscoveryCandidate): string {
  return [
    ...
    candidate.catalogFacts?.setId,
    candidate.catalogFacts?.setName,
    candidate.catalogFacts?.translatedSetName,
    candidate.catalogFacts?.series,
    candidate.catalogFacts?.cardNumber,
    candidate.catalogFacts?.printedTotal,
    candidate.catalogFacts?.language,
    ...
    candidate.catalogFacts?.releaseYear?.toString(),
...
function stripParallelSetTokens(value: string): string {
  let stripped = value;
  const explicitSet = setHintForPrinting(value);
  if (explicitSet) stripped = stripped.replace(new RegExp(`\\b${escapeRegExp(explicitSet)}\\b`, 'ig'), ' ');
  for (const family of PARALLEL_PRINT_SET_FAMILIES) {
    for (const token of family.tokens) {
      stripped = stripped.replace(new RegExp(`\\b${escapeRegExp(token)}\\b`, 'ig'), ' ');
    }
  }
...
function candidateFormatShelfKey(candidate: DiscoveryCandidate): string | undefined {
  const text = normalize([sourceCardText(candidate), candidate.suggestion.lane, ...(candidate.suggestion.sourceTasteTokens ?? [])].filter(Boolean).join(' '));
...
function printingContextKeyFromText(value: string): string | undefined {
  ...
  if (/\bpromo|black star\b/.test(normalized)) context.push('promo');
  if (/\btag team\b/.test(normalized)) context.push('tag-team');
  else if (/\bgx\b/.test(normalized)) context.push('gx');
  else if (/\bvmax\b/.test(normalized)) context.push('vmax');
  else if (/\bvstar\b/.test(normalized)) context.push('vstar');
  else if (/\b(?:mega\s+)?ex\b/.test(normalized)) context.push('ex');
  else if (/\b(?:^|[^a-z])v(?:[^a-z]|$)/.test(normalized)) context.push('v');
  if (/\billustration rare\b|\bsir\b|\bsar\b/.test(normalized)) context.push('premium-art');
  else if (/\bart rare\b|\bar\b|\btrainer gallery\b|\btg\d+\b|\bgg\d+\b/.test(normalized)) context.push('art');
  return context.length > 0 ? context.join('|') : undefined;
}

function setFamilyKeyFromText(value: string): string | undefined {
  const explicitSet = setHintForPrinting(value);
  const normalized = normalize([value, explicitSet].filter(Boolean).join(' '));
  for (const family of PARALLEL_PRINT_SET_FAMILIES) {
    if (family.tokens.some((token) => normalized.includes(normalize(token)))) return family.key;
  }
  return explicitSet ? discoveryNameKey(explicitSet) : undefined;
}

function cardLanguageFromText(value: string): 'JAPANESE' | 'ENGLISH' | undefined {
  return requestedLanguage(value);
}

function chaseReferenceText(chase: Pick<Chase, 'cardName' | 'targetNote'>): string {
  return [chase.cardName, chase.targetNote].filter(Boolean).join(' ');
}

function parallelPrintGroupKeyFromText(value: string): string | undefined {
  const subjectKey = normalizedSubjectIdentity(stripParallelSetTokens(value));
  const setFamilyKey = setFamilyKeyFromText(value);
  const printingContextKey = printingContextKeyFromText(value);
  if (!subjectKey || !setFamilyKey || !printingContextKey) return undefined;
  return `${subjectKey}|${setFamilyKey}|${printingContextKey}`;
}
...
function expectedPokemonSuggestionReferenceImageUrl(candidate: DiscoveryCandidate): string | undefined {
  const sourceCardId = candidate.suggestion.referenceSourceCardId?.trim();
  const requestedNumber = extractRequestedCardNumber([candidate.suggestion.name, candidate.suggestion.evidenceSearchTerm, ...(candidate.suggestion.evidenceAliases ?? [])].filter(Boolean).join(' '))
    ?? sourceCardId?.split('-').pop()?.trim();
  if (!sourceCardId || !requestedNumber) return undefined;
  const setId = sourceCardId.split('-')[0];
...
function isTrustedSuggestionReferenceImage(candidate: DiscoveryCandidate): boolean {
  ...
  if (
    imageOverride
    && candidate.suggestion.referenceImageUrl === imageOverride.imageUrl
    && candidate.suggestion.referenceSourceName === imageOverride.sourceName
    && candidate.suggestion.referenceSourceCardId === imageOverride.sourceCardId
  ) {
    return true;
  }
  ...
  if (!/^Pokemon TCG(?:\s*\(|$)/i.test(candidate.suggestion.referenceSourceName)) return false;
  return candidate.suggestion.referenceImageUrl === expectedPokemonSuggestionReferenceImageUrl(candidate);
}
...
export function listPriorWeeklyDiscoveryDropsForTargetPeriod(
  userId: string,
  date: Date,
  limit = 12
...
function exactRepeatHistoryByCanonicalId(recentDrops: ScheduledDiscoveryDrop[]): Map<string, ExactRepeatHistoryEntry> {
  const history = new Map<string, ExactRepeatHistoryEntry>();
  for (const [index, drop] of recentDrops.entries()) {
    for (const item of drop.items) {
      const canonicalCardId = item.suggestion.referenceSourceCardId?.trim();
      if (!canonicalCardId) continue;
      const existing = history.get(canonicalCardId);
      if (!existing) {
        history.set(canonicalCardId, {
          canonicalCardId,
          suggestionName: item.suggestion.name,
          mostRecentPeriodShown: drop.periodKey,
          priorAppearances: 1,
          mostRecentIndex: index
        });
      } else {
        existing.priorAppearances += 1;
      }
    }
  }
  return history;
...
function applyExactCardRepeatFreshnessOrdering(
  candidates: DiscoveryCandidate[],
  recentDrops: ScheduledDiscoveryDrop[]
): { orderedCandidates: DiscoveryCandidate[]; repeatHistory: Map<string, ExactRepeatHistoryEntry>; penaltySamples: DiscoveryShelfSelectionResult['rejectionSamples']['EXACT_REPEAT_COOLDOWN'] } {
  const repeatHistory = exactRepeatHistoryByCanonicalId(recentDrops);
  const penaltySamples: DiscoveryShelfSelectionResult['rejectionSamples']['EXACT_REPEAT_COOLDOWN'] = [];
  const fresh: DiscoveryCandidate[] = [];
  const penalized: DiscoveryCandidate[] = [];
  const hardExcluded: DiscoveryCandidate[] = [];

  for (const candidate of candidates) {
    const canonicalCardId = candidate.suggestion.referenceSourceCardId?.trim() ?? candidate.image?.sourceCardId?.trim();
    if (!canonicalCardId) {
      fresh.push(candidate);
      continue;
    }
    const history = repeatHistory.get(canonicalCardId);
    if (!history) {
      fresh.push(candidate);
      continue;
    }
  ...
  return { orderedCandidates: [...fresh, ...penalized, ...hardExcluded], repeatHistory, penaltySamples };
}

function reliableWeeklyDiscoveryMarketEstimate(candidate: DiscoveryCandidate, targetCurrency: SupportedCurrency): { amount: number; currency: SupportedCurrency } | null {
  const sourceCurrency = candidate.displayCurrency ?? targetCurrency;
  if (candidate.typicalRawSoldTotal !== undefined && (candidate.soldSampleSize ?? 0) >= MIN_RAW_MARKET_SAMPLE_SIZE) {
    return {
      amount: convertCurrencyAmount(candidate.typicalRawSoldTotal, sourceCurrency, targetCurrency),
      currency: targetCurrency
    };
  }
  if (candidate.typicalRawAskingTotal !== undefined && (candidate.marketSampleSize ?? 0) >= MIN_ASK_ONLY_MARKET_SAMPLE_SIZE) {
    return {
      amount: convertCurrencyAmount(candidate.typicalRawAskingTotal, sourceCurrency, targetCurrency),
      currency: targetCurrency
    };
  }
  return null;
...
function weeklyDiscoveryValueFloor(currency: SupportedCurrency): number {
...
function collapseAdjacentRepeatedNamePhrases(value: string): string {
  const words = value.replace(/\s+/g, ' ').trim().split(' ').filter(Boolean);
  const normalizedWords = words.map((word) => normalize(word).replace(/[^a-z0-9]+/g, ''));
  const output: string[] = [];
  ...
  while (index < words.length) {
    ...
    const maxLength = Math.min(4, Math.floor((words.length - index) / 2));
    for (let length = 1; length <= maxLength; length += 1) {
      const left = normalizedWords.slice(index, index + length);
      const right = normalizedWords.slice(index + length, index + length * 2);
      if (left.every((word, offset) => word.length > 0 && word === right[offset])) {
        repeatedLength = length;
        break;
      }
    }
    if (repeatedLength > 0) {
      const phrase = words.slice(index, index + repeatedLength);
      const normalizedPhrase = normalizedWords.slice(index, index + repeatedLength);
      output.push(...phrase);
      normalizedOutput.push(...normalizedPhrase);
      index += repeatedLength;
      while (
        index + repeatedLength <= words.length
        && normalizedWords.slice(index, index + repeatedLength).every((word, offset) => word === normalizedPhrase[offset])
      ) {
        index += repeatedLength;
      }
      continue;
    ...
    normalizedOutput.push(normalizedWords[index]!);
    index += 1;
  }
...
function scheduledDropItemsFromCandidates(candidates: DiscoveryCandidate[], currency: SupportedCurrency): ScheduledDiscoveryDropItem[] {
  return candidates.map((candidate, index) => ({
    ...
    imageSourceKind: scheduledShelfImageFromCandidate(candidate)?.sourceKind,
    market: {
      ...
      soldSampleSize: candidate.soldSampleSize,
      listing: (candidate.sourceStatus === 'RATE_LIMITED' || candidate.sourceStatus === 'TIMEOUT' || candidate.sourceStatus === 'ERROR') ? undefined : candidate.listing
        ? {
...
function hasProviderBackedScheduledDisplayName(item: ScheduledDiscoveryDropItem): boolean {
  return !!scheduledItemCanonicalId(item)
    && hasTrustedReferenceImage(item)
    && isAllowlistedCatalogueReferenceSource(item.imageSourceName ?? item.suggestion.referenceSourceName)
...
export function validatePublishableDiscoveryShelf(items: ScheduledDiscoveryDropItem[], expectedSize = DISCOVERY_WEEKLY_DROP_SIZE): DiscoveryShelfValidationFailure[] {
  ...
  const badNamePositions = items
    .filter((item) =>
      isGenericDiscoveryCardTitle(item.suggestion.name)
      || (isMarketplaceStyleDiscoveryName(item.suggestion.name) && !hasProviderBackedScheduledDisplayName(item))
    )
...
function emptyDiscoveryShelfRejectionCounts(): Record<DiscoveryShelfSelectionRejectionCode, number> {
  return {
    MISSING_CANONICAL_ID: 0,
    BAD_IMAGE: 0,
    BAD_DISPLAY_NAME: 0,
...
type DiscoveryShelfSelectionRejection = {
  code: DiscoveryShelfSelectionRejectionCode;
  matchedKey?: string;
  matchingVaultCard?: string;
  candidateSet?: string;
  candidateLanguage?: string;
...
type WeeklyShelfSelectionState = {
...
function emptyWeeklyShelfSelectionState(): WeeklyShelfSelectionState {
  return {
...
function normalWeeklyShelfCapLimits(): WeeklyShelfCapLimits {
  return {
...
function parallelPrintVaultEntries(chases: Chase[]): ParallelPrintVaultEntry[] {
  const entries: Array<ParallelPrintVaultEntry | null> = chases.map((chase) => {
    const text = chaseReferenceText(chase);
    const key = parallelPrintGroupKeyFromText(text);
    return key
      ? {
          chaseName: chase.cardName,
          key,
          setKey: setFamilyKeyFromText(text),
          language: cardLanguageFromText(text)
        }
      : null;
  });
  return entries.filter((entry): entry is ParallelPrintVaultEntry => entry !== null);
}

function candidateParallelPrintRejection(candidate: DiscoveryCandidate, vaultEntries: ParallelPrintVaultEntry[]): DiscoveryShelfSelectionRejection | null {
  if (vaultEntries.length === 0) return null;
  const candidateText = sourceCardText(candidate);
  const candidateKey = parallelPrintGroupKeyFromText(candidateText);
  if (!candidateKey) return null;
  const candidateLanguage = cardLanguageFromText(candidateText);
  const candidateSet = setFamilyKeyFromText(candidateText);
  if (!candidateLanguage || !candidateSet) return null;
  for (const vaultEntry of vaultEntries) {
    if (vaultEntry.key !== candidateKey) continue;
    if (!vaultEntry.language || vaultEntry.language === candidateLanguage) continue;
    return {
      code: 'VAULT_PARALLEL_PRINT',
      matchedKey: vaultEntry.key,
      matchingVaultCard: vaultEntry.chaseName,
      candidateSet,
      candidateLanguage,
      vaultSet: vaultEntry.setKey,
      vaultLanguage: vaultEntry.language
    };
  }
  return null;
}
...
function candidateShelfCapRejection(
  ...
  if (familyKey && (selectionState.familyCounts.get(familyKey) ?? 0) >= limits.familyCap) {
    return { code: 'FAMILY_SHELF_CAP', matchedKey: familyKey };
  }
  const formatKey = candidateFormatShelfKey(candidate);
  if (formatKey && (selectionState.formatCounts.get(formatKey) ?? 0) >= limits.formatCap) {
    return { code: 'FORMAT_SHELF_CAP', matchedKey: formatKey };
  }
  const laneKey = candidateLaneShelfKey(candidate);
...
function appendCapRecoverySelections(
  ...
  const recoveryReserve = [...capRejectedCandidates].sort((left, right) =>
    capRecoveryCandidateScore(right) - capRecoveryCandidateScore(left)
  ...
  for (let step = 1; step <= maxStep && selected.length < expectedSize; step += 1) {
    const limits = recoveryCapLimitsForStep(step, expectedSize);
    for (const entry of recoveryReserve) {
...
function selectPublishableWeeklyDiscoveryShelf(
  ...
  const capRejectedCandidates: Array<{
    candidate: DiscoveryCandidate;
    item: ScheduledDiscoveryDropItem;
    rejection: DiscoveryShelfSelectionRejection;
    recommendation: DiscoveryRecommendationProfile;
    ...
    if (rejection) {
      ...
      if (rejectionSamples[rejection.code].length < 3) {
        ...
        rejectionSamples[rejection.code].push(selectionDiagnosticSample(candidate, rejection.code, recommendation, {
          ...
          matchingVaultCard: rejection.matchingVaultCard,
          candidateSet: rejection.candidateSet,
          candidateLanguage: rejection.candidateLanguage,
          vaultSet: rejection.vaultSet,
          vaultLanguage: rejection.vaultLanguage
        }));
      }
      continue;
    }
  ...
  const selected = [...resolvedSelected.slice(0, expectedSize)];
  ...
  const capRelaxationSelections: DiscoveryShelfSelectionResult['capRelaxationSelections'] = [];
  ...
  for (const { candidate } of selected) selectedCandidates.push(candidate);
  const selectedMarketResolvedCount = items.filter((item) => item.market.status === 'READY').length;
  ...
  return {
    ...
    inspectedCount,
    qualifiedCount,
    marketResolvedCount: selectedMarketResolvedCount,
    marketIncompleteCount: Math.max(0, items.length - selectedMarketResolvedCount),
    capRelaxationSelections,
    rejectionCounts,
    rejectionSamples
  };
...
function candidateSupplySourceLabel(candidate: DiscoveryCandidate): string {
  return candidate.supplySource ?? 'UNKNOWN';
}

function scheduledItemFromCandidate(candidate: DiscoveryCandidate, currency: SupportedCurrency): ScheduledDiscoveryDropItem | null {
  const [item] = scheduledDropItemsFromCandidates([candidate], currency);
  return item ?? null;
}
...
function candidateHasTrustedPrintingReference(candidate: DiscoveryCandidate, currency: SupportedCurrency): boolean {
  ...
  return !!item
    ...
    && hasTrustedReferenceImage(item)
    && exactPrintingComparison(candidate).ok;
}

function candidateHasCanonicalTrustedReference(candidate: DiscoveryCandidate, currency: SupportedCurrency): boolean {
  return candidateHasTrustedPrintingReference(candidate, currency)
    && !!candidate.weeklyDiscovery?.canonicalReference;
}

function candidateMarketStatus(candidate: DiscoveryCandidate, currency: SupportedCurrency): DiscoveryScheduledMarketStatus {
  const item = scheduledItemFromCandidate(candidate, currency);
  if (!item) return 'MISSING';
  switch (item.market.status) {
    case 'READY':
    case 'THIN':
    case 'PENDING':
    case 'RATE_LIMITED':
    case 'TIMEOUT':
    case 'ERROR':
      return item.market.status;
    default:
      return 'MISSING';
  }
}

function selectDiversityEligibleWeeklyCandidates(
...
function selectMarketShortfallHydrationTargets(
  rerankedReserve: DiscoveryCandidate[],
  currency: SupportedCurrency,
  recentDrops: ScheduledDiscoveryDrop[],
  activeVaultChases: Chase[],
...
function weeklyDiscoveryLocalSupplySufficientForMarketRecovery(readiness: WeeklyDiscoverySupplyReadiness): boolean {
  ...
  return readiness.canonicalReserveCount >= headroomTarget
    ...
    && readiness.selectedShortfall > 0
    && readiness.marketResolvedShortfall > 0;
}

type WeeklyDiscoveryMarketRecoveryOutcome = {
  ran: boolean;
  readyEligibleDelta: number;
  projectedSelectedDelta: number;
  projectedMarketResolvedDelta: number;
...
function buildWeeklyDiscoverySupplyReadiness(
  ...
  const trustedImageCandidates = stableCanonicalIdCandidates.filter((candidate) => candidateHasTrustedCatalogueImage(candidate, currency));
  const canonicalTrustedCandidates = trustedImageCandidates.filter((candidate) => candidateHasCanonicalTrustedReference(candidate, currency));
  const activeVaultEligibleCandidates = canonicalTrustedCandidates.filter((candidate) =>
    !isActiveChaseEchoSuggestion(candidate.suggestion, activeVaultChases) && !candidateParallelPrintRejection(candidate, vaultEntries)
  );
  const hardEligibleCandidates = activeVaultEligibleCandidates.filter((candidate) => {
    const canonicalId = candidate.suggestion.referenceSourceCardId?.trim() ?? candidate.image?.sourceCardId?.trim();
  ...
  const viableAlternativeCount = rerankedReserve.filter((candidate) => {
    const canonicalId = candidate.suggestion.referenceSourceCardId?.trim() ?? candidate.image?.sourceCardId?.trim();
    return !!canonicalId
      && !selection.selectedCandidates.some((selected) => (selected.suggestion.referenceSourceCardId?.trim() ?? selected.image?.sourceCardId?.trim()) === canonicalId)
      && candidateHasCanonicalTrustedReference(candidate, currency)
      && !candidateParallelPrintRejection(candidate, vaultEntries)
      && !isActiveChaseEchoSuggestion(candidate.suggestion, activeVaultChases)
      && !(repeatHistoryForHeadroom.get(canonicalId)?.mostRecentIndex !== undefined
        && repeatHistoryForHeadroom.get(canonicalId)!.mostRecentIndex < WEEKLY_DISCOVERY_EXACT_REPEAT_COOLDOWN_SHELVES);
  }).length;

  const countsBySource: Record<string, number> = {};
  const countsByLane: Record<string, number> = {};
  ...
  const countsByMarketStatus: Record<string, number> = {};
  ...
  return {
    stages: {
      ...generatedStageCounts,
      stableCanonicalIdCandidates: stableCanonicalIdCandidates.length,
      trustedImageCandidates: trustedImageCandidates.length,
      activeVaultEligibleCandidates: activeVaultEligibleCandidates.length,
    ...
    shouldTopOff: selectedShortfall > 0 || marketResolvedShortfall > 0 || !hasViableHeadroom,
    selectedShortfall,
...
function weeklyDiscoveryCandidateOutcome(
  candidate: DiscoveryCandidate,
  selectedCanonicalIds: Set<string>,
  recentRepeatHistory: Map<string, ExactRepeatHistoryEntry>,
  targetCurrency: SupportedCurrency,
...
export async function buildWeeklyDiscoveryFinalizationInput(
  ...
  const allowSourceCatalogForMissingLiveCachedDrop = context.mode === 'LIVE'
    && cachePreferredRetry
    && !hasExistingPublishableDrop;
  const allowHydrationForMissingLiveCachedDrop = allowSourceCatalogForMissingLiveCachedDrop;
  ...
  if (canResumePreparedReserve) {
    ...
    : [];
  const globalSupplementalUniverseCandidates = discovery.hasFullDiscovery
    ? selectDiscoveryUniverseCandidatesForProfile(
        discovery.tasteProfileChases,
        freshExcludedNames,
        supplementalUniverseTarget,
        repeatGuardChases,
        new Set(indexedSupplementalUniverseCandidates.map(topOffCandidateKey))
      )
    : [];
  const supplementalUniverseCandidates = uniqueCandidatesByDisplayName([
    ...indexedSupplementalUniverseCandidates,
    ...globalSupplementalUniverseCandidates
  ]);
  const supplementalCacheCandidates = discovery.hasFullDiscovery
    ...
    : [];
  const supplementalTraitCacheCandidates = discovery.hasFullDiscovery
    ? traitAlignedReliableMarketCacheCandidates(
        marketContext,
        supplementalCacheTarget,
        discovery.tasteProfileChases,
        repeatGuardChases,
  ...
  let firstMarketRecoveryOutcome: WeeklyDiscoveryMarketRecoveryOutcome | undefined;
  if (
    ...
    const hydrationBudgetMs = firstBlockingMarketRecoveryMs(deadlineAtMs, DISCOVERY_WEEKLY_MARKET_HYDRATION_STAGE_TIMEOUT_MS);
    ...
    if (canHydrateMarketShortfall) {
      const marketHydrationDiagnostics: WeeklyMarketHydrationTargetDiagnostic[] = [];
      ...
      if (marketHydrationDiagnostics.length > 0) {
        console.info(`[DiscoveryShelf] ${JSON.stringify({
          event: 'WEEKLY_DISCOVERY_MARKET_RECOVERY_TARGETS',
          userId: context.userId,
          weeklyPeriod,
          stage: 'market-shortfall-hydration',
          samples: marketHydrationDiagnostics.slice(0, 10)
        })}`);
      }
      throwIfAborted(context.abortSignal);
      throwIfDeadlineExpired(deadlineAtMs);
    } else {
      firstMarketRecoveryOutcome = { ran: false, readyEligibleDelta: 0, projectedSelectedDelta: 0, projectedMarketResolvedDelta: 0, remainingMarketShortfall: supplyReadinessBeforeTopOff.marketResolvedShortfall };
      logWeeklyDiscoveryStage({
        event: 'WEEKLY_DISCOVERY_STAGE',
        userId: context.userId,
        weeklyPeriod,
  ...
  const shouldRunSourceTopOff = shouldRunWeeklyDiscoverySourceTopOff(supplyReadinessBeforeTopOff, firstMarketRecoveryOutcome);
  if (discoveryContext.hasFullDiscovery && shouldRunSourceTopOff && !deferExpensiveHydration && hasWeeklyOptionalStageBudget(deadlineAtMs)) {
    ...
    if (topOff.candidates.length > 0) {
      topOffApplied = true;
      finalStageCounts = {
        rawGeneratedSuggestions: baseStageCounts.rawGeneratedSuggestions + topOff.diagnostics.rawGeneratedSuggestions,
        sourceBackedSuggestions: baseStageCounts.sourceBackedSuggestions + topOff.diagnostics.sourceBackedSuggestions,
        globalUniverseConsidered: baseStageCounts.globalUniverseConsidered + topOff.diagnostics.globalUniverseConsidered,
        userUniverseConsidered: baseStageCounts.userUniverseConsidered + topOff.diagnostics.userUniverseConsidered,
        deduplicatedCandidates: uniqueCandidatesByDisplayName([
          ...candidateReserve,
          ...topOff.candidates
        ]).length
      };
      ...
      if (!deferExpensiveHydration) {
        ...
        topOffCanonicalResolvedReserve = await runWeeklyDiscoveryStage({
          userId: context.userId,
          weeklyPeriod,
          stage: 'topoff-expanded-reserve-market-cache-load',
          inputCount: topOffCanonicalResolvedReserve.length,
          counters: topOffMarketCacheDiagnostics,
          stageMetrics: context.stageMetrics
        }, () => Promise.resolve(candidatesFromDiscoveryMarketCache(topOffCanonicalResolvedReserve, {
          userId: context.userId,
          ...marketContext,
          forceRefreshThinSignal: true,
          queueRefreshes: false
        }, topOffMarketCacheDiagnostics)), {
          outputCount: (value) => value.filter((candidate) => candidateMarketStatus(candidate, marketContext.targetCurrency) === 'READY').length
        });
      }
      ...
      logWeeklyDiscoveryReservePruneDiagnostics(context.userId, weeklyPeriod, 'topoff-reserve-prune', topOffPruneDiagnostics);
      throwIfAborted(context.abortSignal);
    }
  } else if (discoveryContext.hasFullDiscovery && supplyReadinessBeforeTopOff.shouldTopOff) {
    logWeeklyDiscoveryStage({
      event: 'WEEKLY_DISCOVERY_STAGE',
      userId: context.userId,
      weeklyPeriod,
...
function persistValidatedWeeklyDiscoveryDrop(
  ...
  if (candidates.length === 0) {
    return {
      ...
      inspectedCount: 0,
      qualifiedCount: 0,
      marketResolvedCount: 0,
      marketIncompleteCount: 0,
...
function persistFinalizedWeeklyDiscoveryDropSnapshot(
  ...
  if (failures.length > 0) {
    ...
    const finalValues = items
      .map((item) => item.market.soldTotal ?? item.market.askingTotal)
    ...
    console.warn(`[DiscoveryShelf] Validation failed for ${userId}\n${JSON.stringify({
      ...
      marketIncompleteCount: selection.marketIncompleteCount,
      ...
      capRelaxationSelections: selection.capRelaxationSelections,
      rejectionCounts: selection.rejectionCounts,
...
export async function discoverCandidatesForUser(
  ...
  const ignoreMarketPriceCeiling = options.ignoreMarketPriceCeiling ?? false;
  ...
  if (latestDrop && latestDrop.items.length > 0) {
    ...
    return {
      ...
      supplyDiagnostics: {
        ...
        globalUniverseConsidered: 0,
        userUniverseConsidered: 0,
        deduplicatedCandidates: orderedScheduledCandidates.length
      }
    };
  }
  ...
  const selectAndEnrich = async () => {
    ...
    const sourceCatalogTargetCount = sourceCatalogExpansionTargetCount(discoverySelectionCount, targetVisibleCount);
    const sourceCatalogSufficientCount = sourceCatalogSufficientSuggestionCount(discoverySelectionCount, targetVisibleCount);
    ...
    const sourceBackedSuggestions = skipSourceCatalogFetch
      ...
      : await runDiscoveryAssemblySubstage(runtimeContext, 'assembly-source-catalog-primary', activeSafeSuggestions.length, async () =>
        expandSourceBackedSuggestions(activeSafeSuggestions, chases, tasteProfileChases, sourceCatalogSufficientCount, repeatGuardChases), {
        outputCount: (value) => value.length,
        counters: {
          targetCount: sourceCatalogSufficientCount,
    ...
    const japaneseSourceBackedSuggestions = skipSourceCatalogFetch
      ...
      : hasFullDiscovery && hasJapaneseWeightedProfile(tasteProfileChases)
        ? await runDiscoveryAssemblySubstage(runtimeContext, 'assembly-source-catalog-japanese', japaneseSourceBackfillParents(tasteProfileChases).length, async () =>
          expandSourceBackedSuggestions(japaneseSourceBackfillParents(tasteProfileChases), chases, tasteProfileChases, Math.min(DISCOVERY_SHELF_PAGE_SIZE, sourceCatalogSufficientCount), repeatGuardChases), {
          outputCount: (value) => value.length,
          counters: {
            targetCount: Math.min(DISCOVERY_SHELF_PAGE_SIZE, sourceCatalogSufficientCount),
            concurrency: DISCOVERY_ENRICHMENT_CONCURRENCY
          }
        })
        : [];
    ...
    const marketContext = {
      ...
      range: ignoreMarketPriceCeiling ? undefined : discoveryMarketRangeFromChases(tasteProfileChases)
    };
    ...
    let concreteSourceBackedSuggestions = uniqueValuesByName([
      ...japaneseSourceBackedSuggestions,
      ...sourceBackedSuggestions,
    ...
    const profileVariantSourceBackedSuggestions = skipSourceCatalogFetch
      ? []
      : hasLearnedProfile && concreteSourceBackedSuggestions.length < sourceCatalogSufficientCount
        ? await runDiscoveryAssemblySubstage(runtimeContext, 'assembly-source-catalog-profile-variant', profileVariantSourceBackfillParents(tasteProfileChases, sourceCatalogTargetCount).length, async () =>
          expandSourceBackedSuggestions(profileVariantSourceBackfillParents(tasteProfileChases, sourceCatalogTargetCount), chases, tasteProfileChases, sourceCatalogSufficientCount, repeatGuardChases), {
          outputCount: (value) => value.length,
          counters: {
            targetCount: sourceCatalogSufficientCount,
            concurrency: DISCOVERY_ENRICHMENT_CONCURRENCY
        ...
        : [];
    if (profileVariantSourceBackedSuggestions.length > 0) {
      concreteSourceBackedSuggestions = uniqueValuesByName([
    ...
    if (!skipSourceCatalogFetch && concreteSourceBackedSuggestions.length < sourceCatalogSufficientCount) {
      ...
      const starterSourceBackedSuggestions = await runDiscoveryAssemblySubstage(runtimeContext, 'assembly-source-catalog-starter-backfill', starterSelection.suggestions.length, async () =>
        expandSourceBackedSuggestions(starterSelection.suggestions, chases, tasteProfileChases, sourceCatalogSufficientCount, repeatGuardChases), {
        outputCount: (value) => value.length,
        counters: {
          targetCount: sourceCatalogSufficientCount,
          concurrency: DISCOVERY_ENRICHMENT_CONCURRENCY
      ...
      concreteSourceBackedSuggestions = backfillSourceBackedDiscoverySuggestions(concreteSourceBackedSuggestions, starterSourceBackedSuggestions, sourceCatalogTargetCount);
    }
    if (!skipSourceCatalogFetch && hasLearnedProfile && concreteSourceBackedSuggestions.length < sourceCatalogSufficientCount) {
      const broadSourceBackedSuggestions = await runDiscoveryAssemblySubstage(runtimeContext, 'assembly-source-catalog-broad-backfill', broadSourceBackfillParents().length, async () =>
        expandSourceBackedSuggestions(broadSourceBackfillParents(), chases, tasteProfileChases, sourceCatalogSufficientCount, repeatGuardChases), {
        outputCount: (value) => value.length,
        counters: {
          targetCount: sourceCatalogSufficientCount,
          concurrency: DISCOVERY_ENRICHMENT_CONCURRENCY
      ...
      concreteSourceBackedSuggestions = backfillSourceBackedDiscoverySuggestions(concreteSourceBackedSuggestions, broadSourceBackedSuggestions, sourceCatalogTargetCount);
    }
    ...
    const weeklyReserveCandidates = hasFullDiscovery && targetVisibleCount >= DISCOVERY_SHELF_PAGE_SIZE
      ? orderCandidatesForCollectorPresentation(
          uniqueCandidatesByDisplayName([
            ...finalCandidates,
            ...scheduledRelevantCandidates,
            ...
            .filter(isDisplayableDiscoveryCandidate),
          tasteProfileChases,
          Math.max(targetVisibleCount, uniqueCandidatesByDisplayName([
            ...finalCandidates,
            ...scheduledRelevantCandidates,
            ...referencedCandidates,
            ...marketCandidates,
    ...
    const deduplicatedCandidates = uniqueCandidatesByDisplayName(enriched).length;
    return {
      lane: selection.lane,
      candidates,
      weeklyReserveCandidates,
      hiddenVaultPickCount,
      supplyDiagnostics: {
        rawGeneratedSuggestions: activeSafeSuggestions.length,
        sourceBackedSuggestions: freshSourceBackedSuggestions.length,
        globalUniverseConsidered: indexedUniverseCandidates.length > 0 ? 0 : universeCandidates.length,
        userUniverseConsidered: indexedUniverseCandidates.length,
  ...
  const weeklyReserveCandidates = preferred.weeklyReserveCandidates;
  return {
    chases,
    tasteProfileChases,
    settings,
...
export async function prepareWeeklyDiscoveryDropForUser(
  ...
  const currentPreparationState = getWeeklyDiscoveryPreparationState(userId, periodKey);
  const cachePreferredRetry = weeklyDiscoveryShouldPreferCachedRetry(currentPreparationState);
  const stageMetrics: WeeklyDiscoveryStageMetric[] = [];
