...
export const health = {
  ...
  async execute(interaction: any) {
    ...
    const coverageHeading = state.isRunning ? '**Current Run Coverage (Source Groups):**' : '**Last Completed Run Coverage (Source Groups):**';
    const coverageSuffix = state.isRunning ? ' So Far' : '';

    lines.push(
      '',
      coverageHeading,
      `**Scope:** ${state.isRunning ? 'Current in-flight source-group snapshot' : 'Historical source-group snapshot from the last completed poller run'}`,
      ...
      `**Oldest Due Group${coverageSuffix}:** ${formatCoverageGroup(coverage.oldestDue)}`,
      `**Oldest Deferred Group${coverageSuffix}:** ${formatCoverageGroup(coverage.oldestDeferred)}`
    );
