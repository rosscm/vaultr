...
export function selectDiscoverySuggestions(focus: string | null, chases: Chase[], count = 3): DiscoverySelection {
  ...
  const ranked = DISCOVERY_CATALOG
    ...
    .map(({ card }) => card);
  const selected = (ranked.length > 0 ? ranked : DISCOVERY_CATALOG.filter((card) => card.starter)).slice(0, count);

  return {
    lane: pickLane(selected),
    suggestions: selected.map(({ name, why, minimumExampleTotalCad }) => ({ name, why, minimumExampleTotalCad }))
  };
}