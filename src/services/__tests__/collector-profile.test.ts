...
describe('collector interest profile', () => {
  ...
  it('selects live weekly ranking mode from collector profile confidence tier', () => {
    ...
    expect(weeklyDiscoveryRankingModeForCollectorProfile({ ...base, confidence: { tier: 'SEED', score: 0.1, reasons: [] } })).toBe('LEGACY');
    expect(weeklyDiscoveryRankingModeForCollectorProfile({ ...base, confidence: { tier: 'EMERGING', score: 0.4, reasons: [] } })).toBe('LEGACY');
    expect(weeklyDiscoveryRankingModeForCollectorProfile({ ...base, confidence: { tier: 'USABLE', score: 0.6, reasons: [] } })).toBe('COLLECTOR_PROFILE_V1');
    expect(weeklyDiscoveryRankingModeForCollectorProfile({ ...base, confidence: { tier: 'STRONG', score: 0.8, reasons: [] } })).toBe('COLLECTOR_PROFILE_V1');
  });
